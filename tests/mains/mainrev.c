...
#define WIDTH 96
#define HEIGHT 96 
#define MAGIC_VALUE 0x424D//0x4D42
#define NUM_PLANE 1
...
FRESULT fr;
UINT bytes_written;
const char* const filename="image.bmp"

void SaveBMP(const uint8_t* image, const char* filename) {
    BMPHeader_t header;
    ...
    header.reserved1 = 0;
    header.reserved2 = 0;
    header.offset = 0; //sizeof(BMPHeader_t)0x0436;
    ...
    for (int y = HEIGHT - 1; y >= 0; y--) {
        for (int x = 0; x < WIDTH; x++) {
            const uint8_t* pixel = &image[(HEIGHT -y- 1) * WIDTH * BYTES_PER_PIXEL + x * BYTES_PER_PIXEL];
...
int main() {
    ...
    while (photo_num <= NUM_PHOTOS) {
        ...
        char filename[20];
        // sprintf(filename, sizeof(filename), "imagen%d.bmp", photo_num);
